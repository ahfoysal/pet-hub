...
export default function CreateProductModal({
  ...
  useEffect(() => {
    ...
    console.log("📊 Variants updated in state:", variants.length, "variants");
    if (variants.length > 0) {
      console.log(
        "📸 First variant images in state:",
        variants[0].images.length,
        "images",
      );
      console.log(
        "📸 First variant in ref after sync:",
        variantsRef.current[0]?.images.length,
        "images",
      );
    }
  }, [variants]);
  ...
  const updateVariant = (tempId: string, field: keyof Variant, value: any) => {
    console.log(
      `🔄 updateVariant called: tempId=${tempId}, field=${field}, value=`,
      value,
    );
  ...
  const handleVariantImageChange = (
    ...
    console.log("🖼️ handleVariantImageChange called for tempId:", tempId);
    const variant = variants.find((v) => v.tempId === tempId);
    if (!variant) {
    ...
    console.log("Current variant images:", variant.images.length);
    ...
    console.log("💾 About to update - newImages.length:", newImages.length);
  ...
  const createAllVariants = async (productId: string) => {
    ...
    console.log("Total variants to create:", currentVariants.length);
    console.log("✅ Using variantsRef to avoid stale closure bug!");
    console.log("🔍 DEBUG: variantsRef.current content:", currentVariants);
    console.log(
      "🔍 DEBUG: First variant image count from ref:",
      currentVariants[0]?.images?.length,
    );
    ...
    for (let i = 0; i < currentVariants.length; i++) {
      ...
      console.log(
        `\n--- Processing variant ${i + 1}/${currentVariants.length} ---`,
      );
      ...
      console.log(`Adding ${variant.images.length} images`);
      ...
      try {
        console.log("Sending variant creation request...");
