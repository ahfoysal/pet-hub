...
import { useGetPetHotelsQuery } from "@/redux/features/api/dashboard/admin/dashboard/adminDashboardApi";
import { PetHotelItem } from "@/types/dashboard/admin/dashboard/adminDashboardType";
// import UserDetailsModal from "./UserDetailsModal"; // We'll assume UserDetailsModal works for hotels or needs adaptation.
import TablePagination from "./TablePagination";

export default function PetHotelsClient() {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedUser, setSelectedUser] = useState<PetHotelItem | null>(null);
  const [page, setPage] = useState(1);
  const itemsPerPage = 20;

  const { data, isLoading, isError } = useGetPetHotelsQuery({
    search: searchTerm,
    limit: itemsPerPage,
    page,
  });
  ...
  return (
    <div className="size-full bg-[#f2f4f8] -m-6 p-6 min-h-[calc(100vh-80px)]" data-name="Pet Hotels">
      {/* Container */}
        ...
        <div className="flex flex-col gap-[8px] h-[68px] w-full" data-name="Container">
          ...
          <div className="h-[24px] w-full" data-name="Paragraph">
            <p className="font-['Arimo',sans-serif] font-normal leading-[24px] text-[#4a5565] text-[16px] m-0">
              Manage all pet boarding and accommodation providers
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[1090px]">
            <thead>
              <tr className="bg-[#f8fafc] border-b border-[#e2e8f0] h-[60px]">
                <th className="font-['Inter',sans-serif] font-semibold leading-[16px] px-[10px] text-[#314158] text-[12px] tracking-[0.6px] uppercase align-middle pl-[20px]">Provider</th>
                ...
                <th className="font-['Inter',sans-serif] font-semibold leading-[16px] px-[10px] text-[#314158] text-[12px] tracking-[0.6px] uppercase align-middle">Rooms</th>
                <th className="font-['Inter',sans-serif] font-semibold leading-[16px] px-[10px] text-[#314158] text-[12px] tracking-[0.6px] uppercase align-middle">Total Earnings</th>
                  ...
                  >
                    ...
                    <td className="px-[10px] align-middle">
                      <span className="font-['Nunito',sans-serif] font-semibold leading-[24px] text-[#00a63e] text-[16px]">
                        ${(hotel.totalEarnings || 0).toLocaleString()}
                      </span>
                    </td>
                    <td className="px-[10px] align-middle">
