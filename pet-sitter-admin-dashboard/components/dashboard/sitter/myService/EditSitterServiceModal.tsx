...
export default function EditSitterServiceModal({
  ...
  useEffect(() => {
    if (service && isOpen) {
      const priceInDollars = (Number(service.price) / 100).toFixed(0);
      setFormData({
        name: service.name,
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center font-arimo p-4">
      ...
      <div className="relative bg-white w-[660px] rounded-[34px] shadow-[0px_0px_12px_0px_rgba(0,0,0,0.1)] flex flex-col gap-6 border border-[#f4f4f4] z-10 max-h-[95vh] overflow-y-auto overflow-x-hidden animate-in fade-in zoom-in duration-200">
        ...
        {/* Close Icon / Labels Header */}
        <div className="absolute top-8 left-8 right-8 flex justify-between items-start z-20 pointer-events-none">
          <div className="flex gap-8 pointer-events-auto bg-white/40 backdrop-blur-sm px-3 py-1 rounded-full">
            <div className="flex gap-2">
              <span className="text-sm font-normal text-[#667085]">Service ID</span>
              <span className="text-sm font-bold text-[#101828]">PRD-{service.id.slice(-3).toUpperCase()}</span>
            </div>
            <div className="flex gap-2">
              <span className="text-sm font-normal text-[#667085]">Date</span>
              <span className="text-sm font-bold text-[#101828]">14-Oct-2026</span>
            </div>
          </div>
          <button 
            type="button"
            onClick={handleClose}
            className="p-1 text-[#667085] hover:text-[#ff7176] transition-colors pointer-events-auto"
          >
            <X size={24} />
          </button>
        </div>
