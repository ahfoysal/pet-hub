...
export default function DocumentsTab({ docsData, onFileUpload, isUploading }: DocumentsTabProps) {
  ...
  const docItems = [
    { id: "ownerId", title: "National ID / Passport", status: "Verified", date: "2024-01-15", url: docsData?.data?.ownerId },
    { id: "businessLicense", title: "Business License (Optional)", status: "Verified", date: "2024-01-15", url: docsData?.data?.businessLicense },
    { id: "hotelPhotos", title: "Sitter / Facility Photos", status: "Verified", date: "2024-01-15", url: docsData?.data?.hotelPhotos },
    { id: "insuranceCertificate", title: "Insurance Certificate (Optional)", status: "Pending", date: "-", url: docsData?.data?.insuranceCertificate },
  ];
  ...
  return (
    <div className="bg-white border border-[#e5e7eb] rounded-[14px] p-[25px] shadow-sm animate-in fade-in duration-300 font-arimo relative">
      ...
      {/* Hidden inputs for each doc type */}
      {docItems.map(doc => (
        <input 
          key={`input-${doc.id}`}
          id={`upload-${doc.id}`}
