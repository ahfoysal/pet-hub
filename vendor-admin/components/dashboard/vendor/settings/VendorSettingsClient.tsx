...
} from "@/redux/features/api/dashboard/vendor/settings/vendorSettingsApi";
...
export default function VendorSettingsClient() {
  ...
  // API Hooks
  const { data: profileData, isLoading: isProfileLoading, refetch: refetchProfile } = useGetVendorProfileQuery(undefined);
  const [updateProfile, { isLoading: isUpdatingProfile }] = useUpdateVendorProfileMutation();

  const { data: notifyData, isLoading: isNotifyLoading, refetch: refetchNotify } = useGetVendorNotificationSettingsQuery();
  const [updateNotify, { isLoading: isUpdatingNotify }] = useUpdateVendorNotificationSettingsMutation();

  const { data: bankingData, isLoading: isBankingLoading, refetch: refetchBanking } = useGetVendorBankingInfoQuery();
  const [updateBanking, { isLoading: isUpdatingBanking }] = useUpdateVendorBankingInfoMutation();

  const { data: docsData, isLoading: isDocsLoading, refetch: refetchDocs } = useGetVendorDocumentsQuery();
  const [uploadDocs, { isLoading: isUploadingDocs }] = useUploadVendorDocumentsMutation();
  ...
  return (
    <div className="space-y-[25px] pb-10 font-arimo bg-[#f2f4f8]">
      <PageHeader 
        title="Settings" 
        subtitle="Manage your account and vendor settings" 
      />
