...
} from "@/redux/features/api/dashboard/hotel/settings/hotelSettingsApi";
...
export default function HotelSettingsClient() {
  ...
  // API Hooks
  const { data: profileData, isLoading: isProfileLoading, refetch: refetchProfile } = useGetHotelSettingsProfileQuery();
  const [updateProfile, { isLoading: isUpdatingProfile }] = useUpdateHotelSettingsProfileMutation();

  const { data: notifyData, isLoading: isNotifyLoading, refetch: refetchNotify } = useGetHotelNotificationSettingsQuery();
  const [updateNotify, { isLoading: isUpdatingNotify }] = useUpdateHotelNotificationSettingsMutation();

  const { data: bankingData, isLoading: isBankingLoading, refetch: refetchBanking } = useGetHotelBankingInfoQuery();
  const [updateBanking, { isLoading: isUpdatingBanking }] = useUpdateHotelBankingInfoMutation();

  const { data: docsData, isLoading: isDocsLoading, refetch: refetchDocs } = useGetHotelDocumentsQuery();
  const [uploadDocs, { isLoading: isUploadingDocs }] = useUploadHotelDocumentsMutation();
  ...
  return (
    <div className="space-y-[25px] pb-10 font-arimo bg-[#f2f4f8]">
      <PageHeader 
        title="Settings" 
        subtitle="Manage your account and hotel settings" 
      />
